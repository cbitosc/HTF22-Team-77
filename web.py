...
def draw_main_page():
    ...
    st.write(
        """
        In modern day world, with the advent of rapid technological development , we must all take a moment to pause, step back, and take into consideration of all difficulties faced by the specially abled. 

It is the perogative of the up and coming software developers to accomodate each and everyone's interests in society thereby becoming changing makers of the future. 
...
Sign Language is a form of communication used primarily by people hard of hearing or deaf. This type of gesture-based language allows people to convey ideas and thoughts easily overcoming the barriers caused by difficulties from hearing issues.
    ...
    current_cam = 1
    cam_switch = st.checkbox('Start Camera')
    ...
    st.write('Click `Run` to start. The sign table can be found on the left. `del` sign is used before entering a duplicate symbol; `space` is used to end a word.')
...
if len(pages):
    pages.insert(0, "Sign Language Detection System")
    st.sidebar.title("Sign Recognition")
